...
class VisualFrontend(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.frontend3D(x)  # Output: [B, frontend_nout, T, H//4, W//4]
        ...
        x = x.contiguous().view(B * T, x.size(2), x.size(3), x.size(4))  # [B*T, frontend_nout, H//4, W//4]
        
        # Process through ResNet trunk
        x = self.resnet_trunk(x)  # [B*T, backend_out]
        
        # Reshape back to separate batch and time dimensions
        x = x.view(B, T, -1)  # [B, T, backend_out]
...
class Lipreading(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        if isinstance(self.encoder, ConformerEncoder):
            ...
            # Forward through Conformer - output shape: [B, T, hidden_dim]
            x, _ = self.encoder(x, padding_mask)
            # Apply output projection - output shape: [B, T, num_classes]
            x = self.output_layer(x)
        else:
            # Forward through TCN or DenseTCN - output shape: [B, T, num_classes]
